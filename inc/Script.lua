...
local Get_info ="𖡨 أهلاً عزيزي تم مسح جميع معلوماتك "
return sendMsg(msg.chat_id_,msg.id_,Get_info)    
end,nil)
return false
end
...
if mtwren == 0 then  return "𖡨 عذراً لا يوجد مطورين في البوت ." end
...
if not msg.SudoBase then return "𖡨هذا الامر يخص {المطور} فقط  \n" end
...
return "𖡨هذا الامر يخص {المطور الاساسي} فقط  \n" 
end
...
sendMsg(msg.chat_id_,msg.id_,"𖡨 عذراً النسخه الاحتياطيه هذا ليست للبوت » ["..Bot_User.."]  \n")
end
else 
sendMsg(msg.chat_id_,msg.id_,'𖡨 عذراً اسم الملف غير مدعوم للنظام او لا يتوافق مع سورس ويجز يرجاء جلب الملف الاصلي الذي قمت بسحبه وبدون تعديل ع الاسم\n')
end  
else
sendMsg(msg.chat_id_,msg.id_,'𖡨 عذراً الملف ليس بصيغه Json !?\n')
end 
else
sendMsg(msg.chat_id_,msg.id_,'𖡨 عذراً هذا ليس ملف النسحه الاحتياطيه للمجموعات\n')
end 
end,nil)
else 
...
if not msg.SudoBase then return"𖡨هذا الامر يخص {المطور الاساسي} فقط  \n" end
return '𖡨 الاحصائيات . \n\n𖡨عدد المجموعات المفعله : '..redis:scard(wegz..'group:ids')..'\nعدد المشتركين في البوت : '..redis:scard(wegz..'users')..'\n'
end
---------------[End Function data] -----------------------
...
return '𖡨لا يوجد ردود مضافه حاليا \n' 
end
local ii = 1
local message = '𖡨ردود البوت في المجموعه  :\n\n'
...
local ii = 1
local message = '𖡨الردود العامه في البوت :   :\n\n'
for i=1, #photo 	do message = message ..ii..' - *{* '..	photo[i]..' *}_*( صوره ) \n' 	ii = ii + 1 end
for i=1, #names 	do message = message ..ii..' - *{* '..	names[i]..' *}_*( نص ) \n'  	ii = ii + 1 end
for i=1, #voice 	do message = message ..ii..' - *{* '..	voice[i]..' *}_*( بصمه ) \n' 	ii = ii + 1 end
for i=1, #imation 	do message = message ..ii..' - *{* '..imation[i]..' *}_*( متحركه ) \n'ii = ii + 1 end
...
𖡨↞❬أوامر المسح❭
———————wegz—————
𖡨مسح الادمنـيـه «» ❬لمسح الادمنيه❭
𖡨مسح المميزين «» ❬لمسح المميزين❭
...
return '𖡨عدد سحكاتك ⇜ ❪ '..(redis:get(wegz..':edited:'..msg.chat_id_..':'..msg.sender_user_id_) or 0)..' ❫ \n'
end
...
if not msg.Kara then return "𖡨هذا الامر يخص {المنشئ الاساسي,المطور,المطور الاساسي} فقط ." end
...
𖡨3- ارسل تفعيل وسيتم تفعيل البوت ورفع مشرفي القررب تلقائياً .
...
𖡨1- اضف البوت الى المجموعه
𖡨2- ارفع البوت مشرف في المجموعه
𖡨③- ارسل تفعيل وسيتم تفعيل البوت ورفع مشرفي القررب تلقائياً .
